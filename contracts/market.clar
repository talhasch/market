...
(define-public (cancel-order (order-id uint) (ft <ft-trait>))
    (let (
        ...
        (unwrap! (contract-call? ft transfer (get bid order) self tx-sender none) err-ft-transfer-error)
...
(define-public (fill-order (order-id uint) (ft <ft-trait>))
  (let (
            (order (unwrap! (map-get? orders { order-id: order-id }) err-order-not-found))
            (bid (get bid order))
            (fee (/ (* bid (var-get fee-pct)) u10000))
        ) 
        (asserts! (not (is-eq tx-sender (get owner order))) err-your-order)
        (asserts! (is-eq (contract-of ft) (get ft order)) err-incorrect-ft)
        (unwrap! (stx-transfer? (- (get ask order) fee) tx-sender (get owner order)) err-coin-transfer-error)
        (unwrap! (stx-transfer? fee tx-sender (var-get fee-to)) err-coin-transfer-error)
        (unwrap! (contract-call? ft transfer bid self tx-sender none) err-ft-transfer-error)
        (map-delete orders {order-id: order-id})
